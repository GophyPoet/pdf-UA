...
Самое простое — двойной клик по лаунчеру в проводнике:

* Linux / macOS: `./run.sh`
* Windows: `run.bat`

Лаунчер сам доустановит pip-зависимости, проверит наличие `soffice`
и `tesseract`, поднимет Flask и откроет вкладку браузера.
Порт можно переопределить переменной окружения `PDFUA_PORT`.

Эквивалент вручную:

```bash
python -m pdfua.cli serve --host 127.0.0.1 --port 8000 --open
```

Без флага `--open` сервер всё равно поднимется — открыть в браузере
`http://127.0.0.1:8000/`. Страница позволяет перетащить файл, увидеть
текущий этап, логи, финальный отчёт по правилам и скачать ODT / PDF /
JSON-отчёт / TXT-отчёт.
